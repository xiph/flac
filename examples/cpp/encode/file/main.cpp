...
static FLAC__uint32 total_samples = 0; /* can use a 32-bit number due to WAVE size limitations */
...
int main(int argc, char *argv[])
{
	...
	sample_rate = ((((((FLAC__uint32)buffer[27] << 8) | buffer[26]) << 8) | buffer[25]) << 8) | buffer[24];
	channels = 2;
	bps = 16;
	total_samples = (((((((FLAC__uint32)buffer[43] << 8) | buffer[42]) << 8) | buffer[41]) << 8) | buffer[40]) / 4;
...
void OurEncoder::progress_callback(FLAC__uint64 bytes_written, FLAC__uint64 samples_written, FLAC__uint32 frames_written, FLAC__uint32 total_frames_estimate)
{
