...
static FLAC__bool die_s_(const char *msg, const FLAC::Decoder::Stream *decoder)
{
	...
	printf(", state = %u (%s)\n", (FLAC__uint32)((::FLAC__StreamDecoderState)state), state.as_cstring());
...
public:
	Layer layer_;
	FLAC__uint32 current_metadata_number_;
	bool ignore_errors_;
...
static bool test_stream_decoder(Layer layer, bool is_ogg)
{
	...
	{
		FLAC__uint32 channels = decoder->get_channels();
	...
	{
		FLAC__uint32 bits_per_sample = decoder->get_bits_per_sample();
	...
	{
		FLAC__uint32 sample_rate = decoder->get_sample_rate();
	...
	{
		FLAC__uint32 blocksize = decoder->get_blocksize();
