...
static FLAC__bool local__parse_cue_(const char *s, const char *end, FLAC__uint32 *track, FLAC__uint32 *indx)
{
	FLAC__bool got_track = false, got_index = false;
	FLAC__uint32 t = 0, i = 0;
	char c;
 ...
 */
static FLAC__uint64 local__find_closest_cue_(const FLAC__StreamMetadata_CueSheet *cuesheet, FLAC__uint32 track, FLAC__uint32 indx, FLAC__uint64 total_samples, FLAC__bool look_forward)
{
	int t, i;
	if(look_forward) {
...
void flac__utils_canonicalize_skip_until_specification(utils__SkipUntilSpecification *spec, FLAC__uint32 sample_rate)
{
	FLAC__ASSERT(0 != spec);
...
FLAC__bool flac__utils_set_channel_mask_tag(FLAC__StreamMetadata *object, FLAC__uint32 channel_mask)
{
	...
	if((entry.length = flac_snprintf(tag, sizeof(tag), "%s=0x%04X", CHANNEL_MASK_TAG, (FLAC__uint32)channel_mask)) >= sizeof(tag))
		return false;
