...
typedef struct {
	...
	FLAC__uint32 bits_per_sample; /* width of sample point, including 'shift' bits, valid bps is bits_per_sample-shift */
	FLAC__uint32 shift; /* # of LSBs samples have been shifted left by */
	FLAC__uint32 bytes_per_wide_sample; /* for convenience, always == channels*((bps+7)/8), or 0 if N/A to input format (like FLAC) */
...
typedef struct {
	...
	FLAC__uint32 stats_frames_interval;
	FLAC__uint32 old_frames_written;
 ...
 * local routines
 */
static FLAC__bool EncoderSession_construct(EncoderSession *e, encode_options_t options, FLAC__off_t infilesize, FILE *infile, const char *infilename, const char *outfilename, const FLAC__byte *lookahead, FLAC__uint32 lookahead_length);
...
static FLAC__bool EncoderSession_process(EncoderSession *e, const FLAC__int32 * const buffer[], FLAC__uint32 samples);
...
static FLAC__bool canonicalize_until_specification(utils__SkipUntilSpecification *spec, const char *inbasefilename, FLAC__uint32 sample_rate, FLAC__uint64 skip, FLAC__uint64 total_samples_in_input);
static FLAC__bool verify_metadata(const EncoderSession *e, FLAC__StreamMetadata **metadata, FLAC__uint32 num_metadata);
static FLAC__bool format_input(FLAC__int32 *dest[], FLAC__uint32 wide_samples, FLAC__bool is_big_endian, FLAC__bool is_unsigned_samples, FLAC__uint32 channels, FLAC__uint32 bps, FLAC__uint32 shift, size_t *channel_map);
static void encoder_progress_callback(const FLAC__StreamEncoder *encoder, FLAC__uint64 bytes_written, FLAC__uint64 samples_written, FLAC__uint32 frames_written, FLAC__uint32 total_frames_estimate, void *client_data);
...
static FLAC__bool parse_cuesheet(FLAC__StreamMetadata **cuesheet, const char *cuesheet_filename, const char *inbasefilename, FLAC__uint32 sample_rate, FLAC__bool is_cdda, FLAC__uint64 lead_out_offset, FLAC__bool treat_warnings_as_errors);
...
static FLAC__uint32 count_channel_mask_bits(FLAC__uint32 mask);
#if 0
static FLAC__uint32 limit_channel_mask(FLAC__uint32 mask, FLAC__uint32 channels);
#endif
...
static FLAC__bool get_sample_info_wave(EncoderSession *e, encode_options_t options)
{
	...
	FLAC__uint32 sample_rate = 0, channels = 0, bps = 0, shift = 0;
	...
	while(!feof(e->fin) && !got_data_chunk) {
		...
		if(e->format == FORMAT_RF64 && !memcmp(chunk_id, "ds64", 4)) { /* RF64 64-bit sizes chunk */
			 ...
			 *
			 * Data is supposed to be FLAC__uint32 for bps <= 8 else signed.
			 */
			...
			if(!options.format_options.iff.foreign_metadata) {
				if(e->format != FORMAT_WAVE64)
					...
					flac__utils_printf(stderr, 1, "%s: WARNING: skipping unknown chunk %02X%02X%02X%02X-%02X%02X-%02X%02X-%02X%02X-%02X%02X%02X%02X%02X%02X (use --keep-foreign-metadata to keep)\n",
						e->inbasefilename,
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[3],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[2],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[1],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[0],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[5],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[4],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[7],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[6],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[9],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[8],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[10],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[11],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[12],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[13],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[14],
						(FLAC__uint32)((const FLAC__uint8 *)chunk_id)[15]
					);
				if(e->treat_warnings_as_errors)
					return false;
			}
...
int flac__encode_file(FILE *infile, FLAC__off_t infilesize, const char *infilename, const char *outfilename, const FLAC__byte *lookahead, FLAC__uint32 lookahead_length, encode_options_t options)
{
...

	{
		...
		FLAC__uint32 consecutive_eos_count = 0;
		FLAC__uint32 align_remainder = 0;
		...
		if(options.sector_align && (options.format != FORMAT_RAW || infilesize >=0)) { /* for RAW, need to know the filesize */
			...
			align_remainder = (FLAC__uint32)(encoder_session.total_samples_to_encode % 588);
		...
		if(skip > 0) {
			switch(options.format) {
				case FORMAT_RAW:
					{
						FLAC__uint32 skip_bytes = encoder_session.info.bytes_per_wide_sample * (FLAC__uint32)skip;
				...
				if(infilesize < 0) {
					...
					while(!feof(infile)) {
						if(lookahead_length > 0) {
							...
							bytes_read = fread(ubuffer.u8+lookahead_length, sizeof(FLAC__uint8), CHUNK_OF_SAMPLES * encoder_session.info.bytes_per_wide_sample - lookahead_length, infile) + lookahead_length;
							if(ferror(infile)) {
							...
							bytes_read = fread(ubuffer.u8, sizeof(FLAC__uint8), CHUNK_OF_SAMPLES * encoder_session.info.bytes_per_wide_sample, infile);
						...
						if(bytes_read == 0) {
							...
							FLAC__uint32 wide_samples = bytes_read / encoder_session.info.bytes_per_wide_sample;
					...
					while(total_input_bytes_read < max_input_bytes) {
						{
							...
							if(lookahead_length > 0) {
								...
								if(wanted > 0) {
									bytes_read += fread(ubuffer.u8+lookahead_length, sizeof(FLAC__uint8), wanted, infile);
									if(ferror(infile)) {
										flac__utils_printf(stderr, 1, "%s: ERROR during read\n", encoder_session.inbasefilename);
										return EncoderSession_finish_error(&encoder_session);
									}
								...
								bytes_read = fread(ubuffer.u8, sizeof(FLAC__uint8), wanted, infile);
						}
					...
					const size_t bytes_to_read =
						(size_t) min (sizeof (ubuffer.u8),
							min (encoder_session.fmt.iff.data_bytes,
								CHUNK_OF_SAMPLES * (FLAC__uint64) encoder_session.info.bytes_per_wide_sample));
					size_t bytes_read = fread(ubuffer.u8, sizeof(FLAC__uint8), bytes_to_read, infile);
					if(bytes_read == 0) {
						if(ferror(infile)) {
							flac__utils_printf(stderr, 1, "%s: ERROR during read\n", encoder_session.inbasefilename);
							return EncoderSession_finish_error(&encoder_session);
		...
		if(options.sector_align) {
			if(options.is_last_file) {
				FLAC__uint32 wide_samples = 588 - align_remainder;
				if(wide_samples < 588) {
				...
				if(*options.align_reservoir_samples > 0) {
					...
					bytes_read = fread(ubuffer.u8, sizeof(FLAC__uint8), (*options.align_reservoir_samples) * encoder_session.info.bytes_per_wide_sample, infile);
...
FLAC__bool EncoderSession_construct(EncoderSession *e, encode_options_t options, FLAC__off_t infilesize, FILE *infile, const char *infilename, const char *outfilename, const FLAC__byte *lookahead, FLAC__uint32 lookahead_length)
{
	FLAC__uint32 i;
	FLAC__uint32 test = 1;
...
{
	const FLAC__uint32 channels = e->info.channels;
	const FLAC__uint32 bps = e->info.bits_per_sample - e->info.shift;
	const FLAC__uint32 sample_rate = e->info.sample_rate;
	...
	FLAC__uint32 num_metadata = 0, ic;
	...
	if(flac_decoder_data) {
		...
		if(options.padding != 0) {
			...
			padding.length = (FLAC__uint32)(options.padding>0? options.padding : (e->total_samples_to_encode / sample_rate < 20*60? FLAC_ENCODE__DEFAULT_PADDING : FLAC_ENCODE__DEFAULT_PADDING*8)) + (e->replay_gain ? GRABBAG__REPLAYGAIN_MAX_TAG_SPACE_REQUIRED : 0);
...
FLAC__bool EncoderSession_process(EncoderSession *e, const FLAC__int32 * const buffer[], FLAC__uint32 samples)
{
	if(e->replay_gain) {
...
FLAC__bool canonicalize_until_specification(utils__SkipUntilSpecification *spec, const char *inbasefilename, FLAC__uint32 sample_rate, FLAC__uint64 skip, FLAC__uint64 total_samples_in_input)
{
...
FLAC__bool verify_metadata(const EncoderSession *e, FLAC__StreamMetadata **metadata, FLAC__uint32 num_metadata)
{
...
FLAC__bool format_input(FLAC__int32 *dest[], FLAC__uint32 wide_samples, FLAC__bool is_big_endian, FLAC__bool is_unsigned_samples, FLAC__uint32 channels, FLAC__uint32 bps, FLAC__uint32 shift, size_t *channel_map)
{
	FLAC__uint32 wide_sample, sample, channel;
	...
	if(bps == 8) {
		...
		if(is_big_endian != is_big_endian_host_) {
			FLAC__uint8 tmp;
			const FLAC__uint32 bytes = wide_samples * channels * (bps >> 3);
			FLAC__uint32 b;
			for(b = 0; b < bytes; b += 2) {
				tmp = ubuffer.u8[b];
...
void encoder_progress_callback(const FLAC__StreamEncoder *encoder, FLAC__uint64 bytes_written, FLAC__uint64 samples_written, FLAC__uint32 frames_written, FLAC__uint32 total_frames_estimate, void *client_data)
{
...
FLAC__StreamDecoderWriteStatus flac_decoder_write_callback(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
	...
	if(!EncoderSession_process(e, buffer, (FLAC__uint32)n)) {
		print_error_with_state(e, "ERROR during encoding");
...
FLAC__bool parse_cuesheet(FLAC__StreamMetadata **cuesheet, const char *cuesheet_filename, const char *inbasefilename, FLAC__uint32 sample_rate, FLAC__bool is_cdda, FLAC__uint64 lead_out_offset, FLAC__bool treat_warnings_as_errors)
{
	FILE *f;
...
static void print_stats(const EncoderSession *encoder_session)
{
	if(flac__utils_verbosity_ >= 2) {
		...
		if(encoder_session->samples_written == encoder_session->total_samples_to_encode) {
			...
			stats_print_name(2, encoder_session->inbasefilename);
			stats_print_info(2, "%u%% complete, ratio=%s", (FLAC__uint32)floor(encoder_session->progress * 100.0 + 0.5), ratiostr);
		}
	}
}
...
FLAC__uint32 count_channel_mask_bits(FLAC__uint32 mask)
{
...
#if 0
FLAC__uint32 limit_channel_mask(FLAC__uint32 mask, FLAC__uint32 channels)
{
	FLAC__uint32 x = 0x80000000;
	FLAC__uint32 count = count_channel_mask_bits(mask);
