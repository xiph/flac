...
typedef struct {
	...
	FILE *file;
	char filename[512];
	FLAC__uint32 current_metadata_number;
...
static FLAC__uint32 num_expected_;
...
static FLAC__bool test_stream_decoder(Layer layer, FLAC__bool is_ogg)
{
	...
	{
		FLAC__uint32 bits_per_sample = FLAC__stream_decoder_get_bits_per_sample(decoder);
	...
	{
		FLAC__uint32 sample_rate = FLAC__stream_decoder_get_sample_rate(decoder);
	...
	{
		FLAC__uint32 blocksize = FLAC__stream_decoder_get_blocksize(decoder);
	...
	printf("testing FLAC__stream_decoder_get_channel_assignment()... ");
	{
		...
		printf("returned %u (%s)... OK\n", (FLAC__uint32)ca, FLAC__ChannelAssignmentString[ca]);
	}
