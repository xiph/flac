...
const FLAC__uint32 GRABBAG__REPLAYGAIN_MAX_TAG_SPACE_REQUIRED = 190;
/*
...
FLAC__bool grabbag__replaygain_is_valid_sample_frequency(FLAC__uint32 sample_frequency)
{
	return ValidGainFrequency( sample_frequency );
}

FLAC__bool grabbag__replaygain_init(FLAC__uint32 sample_frequency)
{
	title_peak_ = album_peak_ = 0.0;
	return InitGainAnalysis((long)sample_frequency) == INIT_GAIN_ANALYSIS_OK;
}

FLAC__bool grabbag__replaygain_analyze(const FLAC__int32 * const input[], FLAC__bool is_stereo, FLAC__uint32 bps, FLAC__uint32 samples)
{
	...
	static const FLAC__uint32 nbuffer = sizeof(lbuffer) / sizeof(lbuffer[0]);
	...
	if(bps == 16) {
		if(is_stereo) {
			j = 0;
			while(samples > 0) {
				const FLAC__uint32 n = local_min(samples, nbuffer);
				for(i = 0; i < n; i++, j++) {
					s = input[0][j];
...
static FLAC__StreamDecoderWriteStatus write_callback_(const FLAC__StreamDecoder *decoder, const FLAC__Frame *frame, const FLAC__int32 * const buffer[], void *client_data)
{
	...
	const FLAC__uint32 bits_per_sample = frame->header.bits_per_sample;
	const FLAC__uint32 channels = frame->header.channels;
	const FLAC__uint32 sample_rate = frame->header.sample_rate;
	const FLAC__uint32 samples = frame->header.blocksize;
