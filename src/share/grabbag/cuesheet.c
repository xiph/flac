 ...
 */
static FLAC__int64 local__parse_msf_(const char *s, FLAC__uint32 sample_rate)
{
 ...
 */
static FLAC__int64 local__parse_ms_(const char *s, FLAC__uint32 sample_rate)
{
	FLAC__int64 ret, field;
	double x;
	char c, *end;
...
static FLAC__bool local__cuesheet_parse_(FILE *file, const char **error_message, FLAC__uint32 *last_line_read, FLAC__StreamMetadata *cuesheet, FLAC__uint32 sample_rate, FLAC__bool is_cdda, FLAC__uint64 lead_out_offset)
{
	char buffer[4096], *line, *field;
	FLAC__uint32 forced_leadout_track_num = 0;
				...
				*l = '\0';
				...
				if(0 != (field = local__get_field_(&line, /*allow_quotes=*/false))) {
					if(0 == strcmp(field, "FLAC__lead-in")) {
						...
						forced_leadout_track_num = (FLAC__uint32)track_num;
						/*@@@ search for duplicate track number? */
						if(0 == (field = local__get_field_(&line, /*allow_quotes=*/false))) {
...
void grabbag__cuesheet_emit(FILE *file, const FLAC__StreamMetadata *cuesheet, const char *file_reference)
{
	...
	FLAC__uint32 track_num, index_num;
	...
	for(track_num = 0; track_num < cs->num_tracks-1; track_num++) {
		...
		fprintf(file, "  TRACK %02u %s\n", (FLAC__uint32)track->number, track->type == 0? "AUDIO" : "DATA");
		...
		for(index_num = 0; index_num < track->num_indices; index_num++) {
			...
			fprintf(file, "    INDEX %02u ", (FLAC__uint32)indx->number);
			if(cs->is_cd) {
				const FLAC__uint32 logical_frame = (FLAC__uint32)((track->offset + indx->offset) / (44100 / 75));
				FLAC__uint32 m, s, f;
	...
	fprintf(file, "REM FLAC__lead-out %u %" PRIu64 "\n", (FLAC__uint32)cs->tracks[track_num].number, cs->tracks[track_num].offset);
}
